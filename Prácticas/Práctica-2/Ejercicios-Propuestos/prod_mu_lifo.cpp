...
int producir_dato( unsigned i, unsigned j )
{
   assert(i <= np-1);

   this_thread::sleep_for( chrono::milliseconds( aleatorio<min_ms,max_ms>() ));
   const int valor_producido = i*p + j ; //i*p + con_item_prod[i]
   mtx.lock();
...
void funcion_hebra_productora( MRef<ProdConsMu> monitor, unsigned num_hebra )
{
