...
const int
   ...
   num_procesos  = 2*num_filosofos+1 ,   // número de procesos total (por ahora solo hay filo y ten)
...
{
  int valor, cont_sentados = 0 ;  // valor recibido, número de filósofos que tiene sentado
  MPI_Status estado ;             // metadatos de las dos recepciones

  while ( true )
  {
     // Camarero recibe petición de cualquier filosofo
     const int etiq_peticion = (cont_sentados < 4) ? MPI_ANY_TAG : etiq_levantarse;

     MPI_Recv(&valor, 1, MPI_INT, MPI_ANY_SOURCE, etiq_peticion, MPI_COMM_WORLD, &estado);

     // Petición de sentarse
     if (estado.MPI_TAG == etiq_sentarse)
     {
         cont_sentados++;
         cout << "Camarero ha autorizado al filósofo "<< estado.MPI_SOURCE <<" a sentarse. Hay " << cont_sentados << " filósofos sentados" 
         << endl << flush; 

     }

     // Petición de levantarse
     else 
     {
         cont_sentados--;
         cout << "Camarero ha autorizado al filósofo "<< estado.MPI_SOURCE <<" a levantarse de la mesa. Hay " << cont_sentados << " filósofos sentados" 
         << endl << flush;
