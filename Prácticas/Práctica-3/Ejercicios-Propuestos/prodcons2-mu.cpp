...
const int
   ...
   id_min_prod           = 0 ,
   id_max_prod     	 = id_min_prod + np - 1,
   id_buffer             = id_max_prod + 1 ,
   id_min_cons	         = id_buffer + 1 ,
   id_max_cons	         = id_min_cons + nc - 1 ,
   num_procesos_esperado = id_max_cons + 1 ;
...
{
   static int contador = order*k + cont_prod[order] ;
   sleep_for( milliseconds( aleatorio<10,100>()) );
   cont_prod[order]++;
   cout << "Productor "<< order << " ha producido valor " << contador << endl << flush;
   return contador ;
}
// ---------------------------------------------------------------------
...
void funcion_buffer()
{
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      MPI_Recv( &valor, 1, MPI_INT, id_emisor_aceptable, etiqueta_aceptable, MPI_COMM_WORLD, &estado );
...
int main( int argc, char *argv[] )
{
   ...
   if ( num_procesos_esperado == num_procesos_actual )
   {
      ...
      if ( id_min_prod <= id_propio and id_propio <= id_max_prod )
         funcion_productor(id_propio-id_min_prod);
      else if ( id_min_cons <= id_propio and id_propio <= id_max_cons)
